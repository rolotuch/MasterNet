...
namespace MasterNet.Aplicacion.Core
{
    ...
    {        
        ...
        // esto me permite instanciar objetos de tipo espresion con sus respectivos delegate(Func<T, bool>> New<T>(Expression<Func<T, bool>> expression)
        public static Expression<Func<T, bool>> New<T>(Expression<Func<T, bool>> expression)
        {
            return expression;
        }

        //una vez creado la espresion, me permite crear multiples condicionales.
